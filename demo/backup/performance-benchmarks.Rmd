...
curr.recall = rhoR:::getR(curr.kappa, baserate, curr.precision)
simulated.set = rhoR:::prset(precision = curr.precision, recall = curr.recall, length = 10000, baserate = baserate);
```
...
recall = rhoR:::getR(kappa, baserate, precision)

microbenchmark(
  fullSet.r = rhoR:::prset(precision = precision, recall = recall, length = 10000, baserate = baserate),
  fullSet.c = rhoR:::contingency_table(precision = precision, rec = recall, length = 10000, baserate = baserate),
  times = 10, unit = "ms"
)
...
```{r}
codeset = rhoR:::prset(precision = precision, recall = recall, length = 10000, baserate = baserate)
ctset = rhoR:::contingency_table(precision = precision, rec = recall, length = 10000, baserate = baserate)
...
```{r include=FALSE}
# run.bench <- function(oK, fsb, hsl, fsl = 10000, times = 100) {
#   microbenchmark(
#     rho.set = rhoR:::calcRho(observedKappa = oK, fullSetBaserate = fsb, handSetLength = hsl, fullSetLength = fsl),
#     rho.ct = rhoR:::calcRho2(observedKappa = oK, fullSetBaserate = fsb, handSetLength = hsl, fullSetLength = fsl),
#     times = times
#   )
# }
```
